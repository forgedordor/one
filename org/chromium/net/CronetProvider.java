...
public abstract class CronetProvider {
    ...
    private static boolean addCronetProviderFromResourceFile(Context context, fgcy fgcyVar, Set<ffzg> set) {
        ...
        if (string != null && !string.equals(PLAY_SERVICES_CRONET_PROVIDER_CLASS) && !string.equals(GMS_CORE_CRONET_PROVIDER_CLASS) && !string.equals(JAVA_CRONET_PROVIDER_CLASS) && !string.equals(NATIVE_CRONET_PROVIDER_CLASS)) {
            z = true;
            if (!addCronetProviderImplByClassName(context, string, fgcyVar, set, true)) {
                Log.e(TAG, a.a(string, "Unable to instantiate Cronet implementation class ", " that is listed as in the app string resource file under CronetProviderClassName key"));
            }
        }
        return z;
    }

    private static boolean addCronetProviderImplByClassName(Context context, String str, fgcy fgcyVar, Set<ffzg> set, boolean z) throws NoSuchMethodException, SecurityException {
        try {
            ...
            ffzg ffzgVar = new ffzg();
            ffzgVar.a = (CronetProvider) constructor.newInstance(context);
            ffzgVar.b = fgcyVar;
            set.add(ffzgVar);
            return true;
    ...
    static List<ffzg> getAllProviderInfos(Context context) throws NoSuchMethodException, SecurityException {
        LinkedHashSet linkedHashSet = new LinkedHashSet();
        addCronetProviderFromResourceFile(context, fgcy.CRONET_SOURCE_UNSPECIFIED, linkedHashSet);
        fgcy fgcyVar = fgcy.CRONET_SOURCE_PLAY_SERVICES;
        addCronetProviderImplByClassName(context, PLAY_SERVICES_CRONET_PROVIDER_CLASS, fgcyVar, linkedHashSet, false);
        addCronetProviderImplByClassName(context, GMS_CORE_CRONET_PROVIDER_CLASS, fgcyVar, linkedHashSet, false);
        addCronetProviderImplByClassName(context, NATIVE_CRONET_PROVIDER_CLASS, fgcy.CRONET_SOURCE_STATICALLY_LINKED, linkedHashSet, false);
        addCronetProviderImplByClassName(context, JAVA_CRONET_PROVIDER_CLASS, fgcy.CRONET_SOURCE_FALLBACK, linkedHashSet, false);
    ...
    public static List<CronetProvider> getAllProviders(Context context) {
        ...
        Iterator<ffzg> it = getAllProviderInfos(context).iterator();
        while (it.hasNext()) {
    ...
    private static void logReflectiveOperationException(String str, boolean z, Exception exc) {
        if (z) {
            ...
            if (Log.isLoggable(str2, 3)) {
                Log.d(str2, a.a(str, "Tried to load ", " provider class but it wasn't included in the app classpath"));
            }
        }
    }
